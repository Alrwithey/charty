...
// check_permission('manage_assembly');
require_once '../config.php';
$member_type_value = 'assembly';
$page_title = 'إدارة أعضاء الجمعية العمومية';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    header("Location: manage_assembly.php?success=1");
    exit();
}
...
if (isset($_GET['delete'])) {
    ...
    $stmt->execute();
    header("Location: manage_assembly.php?success=2");
    exit();
}
...
<div class="form-container">
    <h2>إضافة عضو جديد للجمعية العمومية</h2>
    <form action="manage_assembly.php" method="POST" class="admin-form" enctype="multipart/form-data">
        <input type="hidden" name="csrf_token" value="<?php echo e($csrf_token); ?>">
        <div class="form-group"><label>الاسم الكامل</label><input type="text" name="name" required></div>
        <div class="form-group"><label>الصفة</label><input type="text" name="position" required></div>
        ...
        <div class="form-group"><label>المدة المتاحة (اختياري)</label><input type="text" name="term_duration"></div>
             ...
             <?php if ($members_result && $members_result->num_rows > 0): ?>
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <a href="manage_assembly.php?delete=<?php echo $member['id']; ?>" class="btn-delete" onclick="return confirm('هل أنت متأكد؟')">حذف</a>
                    </td>
                </tr>
                <?php endwhile; ?>
            <?php else: ?>
